  ...
  if (files?.length) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      thumbnailContainer.onclick = () =>
        videoPlayer.movePlaylistTo(numberOfFiles - (files.length - i));
      ...
      const exponent = Math.min(
        Math.floor(Math.log(size) / Math.log(1024), units.length - 1)
      );
      ...
      video.onloadeddata = function ({ target }) {
        ...
        document.getElementById("total-time").textContent =
          tmg.formatDuration(totalTime);
        span.innerHTML += `<br> Duration: ${tmg.formatDuration(
          video.duration
        )}`;
      };
    }
    if (!videoPlayer) {
      ...
      videoPlayer.playlist = videoPlayer.playlist
